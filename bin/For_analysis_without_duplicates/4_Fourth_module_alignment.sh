...
bwa mem -t 4 -p ../Process/data/wrefgen/wrefgen.fa ../Process/data/anwdvp/***.fq.gz > ../Process/data/wgen_aln/name_archive.sam

##Built a loop for data trimmed to alignment in one comand

for alignment in `ls ../Process/data/anwdvp/ | grep -oE "\w*" | uniq`;
do bwa mem -t 4 -p ../Process/data/wgen/wrefgen.fa ../Process/data/anwdvp/${alignment}.fq.gz > ../Process/data/wgen_aln/${alignment}.sam;
done
