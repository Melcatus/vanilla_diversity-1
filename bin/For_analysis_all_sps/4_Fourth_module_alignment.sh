...
bwa mem -t 4 -p ../Process/data/wrefgen/wrefgen.fa ../Process/data/anasps/***.fq.gz > ../Process/data/wgen_aln/name_archive.sam

##Built a loop for data trimmed to alignment in one comand

for alignment in `ls ../Process/data/anasps/ | grep -oE "\w*" | uniq`;
do bwa mem -t 4 -p ../Process/data/wgen/wrefgen.fa ../Process/data/anasps/${alignment}.fq.gz > ../Process/data/wgen_aln/${alignment}.sam;
done


##Erase incorrect output files if exist. Total of output archives -> 73
